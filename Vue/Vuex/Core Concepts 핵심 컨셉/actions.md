# actions 액션 (비동기?!)
액션은 mutations(변이)와 유사합니다. 몇가지 다른 점은,
...
~~~
액션 핸들러는 저장소 인스턴스의 같은 메소드들/프로퍼티 세트를 드러내는 컨텍스트 객체를 받습니다. 그래서 ``context.commit``을 호출하여 변이(mutations)를 커밋하거나 ``context.state``와 ``context.getters``를 통해 상태와 getters에 접근 할 수 있습니다. 나중에 [모듈](https://vuex.vuejs.org/kr/guide/modules.html)에서 이 컨텍스트 객체가 저장소 인스턴스 자체가 아닌 이유를 알 수 있습니다.  
...
처음 볼 때는 이상해 보일 수 있습니다. 카운트를 증가 시키려면 ``store.commit(`increment`)`` 를 직접 호출하면 어떻습니까? 음, __상태변이는 동기적__ 이어야 한다는 것을 기억하십니까? 액션은 그렇지 않습니다. 액션 내에서 __비동기__ 작업을 수행 할 수 있습니다.
...
~~~
액션의 좀 더 실용적인 예는 __비동기 API호출__ 과 __여러 개의 변이를 커밋__ 하는 장바구니 결제입니다.
~~~js
actions: {
...
``this.$store.dispatch(`xxx`)`` 를 사용하여 컴포넌트에서 액션을 디스채피하거나 컴포넌트 메소드를 ``store.dispatch`` 호출에 매핑하는 ``mapActions`` 헬퍼를 사용할 수 있습니다(루트 ``store`` 주입 필요)
~~~js
...
~~~
마지막으로, JavaScript 기능인 [async/await](https://tc39.es/ecmascript-asyncawait/) 를 사용하면 다음과 같은 작업을 구성 할 수 있습니다.
~~~js
