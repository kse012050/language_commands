...
나머지 요소의 오른쪽 뒤에 쉼표가 있으면 ``SyntaxError``가 발생합니다.
~~~js
var [a, ...b,] = [1, 2, 3];
...
({a, b} = {a: 1, b: 2});
~~~
> __참고__: 할당 문을 둘러싼 ``( .. )``는 선언 없이 객체 리터럴(object literal) 비구조화 할당을 사용할 때 필요한 구문입니다.
...
[내용출처 MDN 구조 분해 할당](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment)