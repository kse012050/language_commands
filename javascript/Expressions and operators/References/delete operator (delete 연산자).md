# delete operator ( delete 연산자 )
``delete`` __연산자__ 는 객체의 속성을 제거합니다. 제거한 객체의 참조를 어디에서도 사용하지 않는다면 나중에 자원을 회수합니다.
...
### 반환 값
``true``. 단, 비엄격 모드에서 속성이 자신의 속성이며 설정 불가능 (en-US)한 경우 ``false``.

### 예외
엄격 모드에서, 속성이 자신의 속성이며 설정 불가능한 경우 ``TypeError``
...
~~~
``var``, ``let``, ``const``로 선언된 변수는 non-configurable 속성으로 구분되며, delete로 삭제될 수 없습니다.
~~~js
var nameOther = 'XYZ';
...
### 엄격 vs. 비엄격 모드
엄격 모드에서 ``delete``로 변수나 함수를 삭제하려고 하면 SyntaxError가 발생합니다.  
...
### delete와 프로토타입 체인
다음 예제에서는 프로토타입 체인에서 동일한 이름의 속성을 사용할 수 있는 동안 객체의 자체 속성을 삭제합니다.
~~~js
function Foo() {
  this.bar = 10;
}
...
// 자신의 속성이 있으므로 true를 반환합니다.
// foo 객체에서 삭제되었습니다.
delete foo.bar;

// foo.bar는 여전히 사용 가능합니다.
// 프로토타입 체인에서 사용할 수 있습니다.
console.log(foo.bar);

// 프로토타입에서 속성을 삭제합니다.
delete Foo.prototype.bar;

// 속성 이후 "정의되지 않음"을 기록합니다.
// 더 이상 상속되지 않음
console.log(foo.bar);
~~~

### 객체 요소 제거하기
배열 요소를 삭제해도 배열 길이는 영향을 받지 않습니다. 이것은 배열의 마지막 요소를 삭제하더라도 유지됩니다.  
  
delete 연산자가 배열 요소를 제거하면 해당 요소는 더 이상 배열에 없습니다. 다음 예에서 tree[3]은 삭제로 제거됩니다.
~~~js
...
~~~
배열 요소가 존재하기를 원하지만 값이 정의되지 않은 경우 삭제 연산자 대신 정의되지 않은 값을 사용하십시오. 다음 예에서 trees[3]에는 정의되지 않은 값이 할당되었지만 배열 요소는 여전히 존재합니다.
~~~js
var trees = ['redwood', 'bay', 'cedar', 'oak', 'maple'];
trees[3] = undefined;
if (3 in trees) {
...
[내용출처 MDN object 요소 제거](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Operators/delete)