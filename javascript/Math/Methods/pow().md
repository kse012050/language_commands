...
## 설명
``Math.pow()``함수는 ``base``의 ``exponent`` 곱, 즉 ``base^exponent``를 반환합니다. ``base``와 ``exponent``는 10진수입니다.
...
[내용출처 MDN 거듭제곱근](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Global_Objects/Math/pow)