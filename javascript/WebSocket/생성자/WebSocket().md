...
- ``url``을 파싱할 수 없음
- ``url``의 스킴이 ``ws``와 ``wss`` 중 하나가 아님
- ``url``에 ``프래그먼트``가 있음
- ``protocols`` 배열의 값들이 고유하지 않거나, WebSocket Protocol 명세가 정의한 ``Sec-WebSocket-Protocol``의 필드 값을 구성하는 요소들의 요구사항에 맞지 않음

[내용출처 MDN WebSocket() 생성자](https://developer.mozilla.org/ko/docs/Web/API/WebSocket/WebSocket)